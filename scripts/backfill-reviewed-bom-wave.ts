...
function buildForwardSql(projects: PreparedProject[], now: string): string {
  ...
  for (const project of projects) {
    ...
    const currentWaveBomGuard = `EXISTS (SELECT 1 FROM boms wave_bom WHERE wave_bom.id = ${sqlString(project.bomId)} AND wave_bom.project_id = ${sqlString(row.id)} AND wave_bom.current_version_id = ${sqlString(project.bomVersionId)} AND wave_bom.created_at = ${sqlString(now)})`;
    ...
    lines.push(`INSERT INTO bom_versions (id, bom_id, version_label, notes, currency, created_by_user_id, created_at) SELECT ${sqlString(project.bomVersionId)}, wave_bom.id, ${sqlString(row.version_label)}, ${sqlString(`Reviewed explicit BOM wave ${wave.wave}; sources pinned to ${definition.revision}.`)}, 'USD', ${sqlString(row.owner_user_id)}, ${sqlString(now)} FROM boms wave_bom WHERE wave_bom.id = ${sqlString(project.bomId)} AND wave_bom.project_id = ${sqlString(row.id)} AND wave_bom.current_version_id = ${sqlString(project.bomVersionId)} AND wave_bom.created_at = ${sqlString(now)};`);
    project.items.forEach((item, index) => {
      ...
      lines.push(`INSERT INTO bom_items (id, bom_version_id, slot_key, description, quantity, unit, target_unit_price_minor, notes, sort_order, extraction_method, completeness, evidence_locator, confidence) SELECT ${sqlString(itemId)}, ${sqlString(project.bomVersionId)}, ${sqlString(item.ref)}, ${sqlString(item.name.slice(0, 500))}, ${sqlNumber(item.quantity)}, 'each', ${sqlNumber(targetUnitPriceMinor)}, ${sqlString(normalizedNotes(item))}, ${index}, 'explicit-bom', ${sqlString(completeness)}, ${sqlString(evidenceLocator)}, ${sqlNumber(confidenceFor(completeness))} WHERE ${currentWaveBomGuard} AND EXISTS (SELECT 1 FROM bom_versions wave_version WHERE wave_version.id = ${sqlString(project.bomVersionId)} AND wave_version.bom_id = ${sqlString(project.bomId)});`);
    });
    for (const evidence of project.evidence) {
      lines.push(`INSERT INTO evidence (id, source_type, source_url, title, publisher, retrieved_at, confidence, content_hash, excerpt, is_demo, created_at) SELECT ${sqlString(evidence.id)}, 'repo', ${sqlString(evidence.sourceUrl)}, ${sqlString(evidence.title)}, ${sqlString(new URL(definition.repo_url).hostname)}, ${sqlString(now)}, 0.98, ${sqlString(`sha256:${evidence.sha256}`)}, ${sqlString(`Explicit BOM source ${evidence.path} at immutable revision ${definition.revision}.`)}, 0, ${sqlString(now)} WHERE ${currentWaveBomGuard};`);
      lines.push(`INSERT INTO evidence_claims (id, evidence_id, entity_type, entity_id, claim_key, claim_value, confidence, created_at) SELECT ${sqlString(evidence.claimId)}, ${sqlString(evidence.id)}, 'project', ${sqlString(row.id)}, 'bom.source', ${sqlString(`${evidence.path}@${definition.revision}`)}, 0.98, ${sqlString(now)} WHERE ${currentWaveBomGuard} AND EXISTS (SELECT 1 FROM evidence wave_evidence WHERE wave_evidence.id = ${sqlString(evidence.id)} AND wave_evidence.created_at = ${sqlString(now)});`);
    }
    lines.push(`UPDATE project_versions SET rpps_json = ${sqlString(JSON.stringify(project.nextRpps))} WHERE id = ${sqlString(row.current_version_id)} AND project_id = ${sqlString(row.id)} AND rpps_json = ${sqlString(row.rpps_json)} AND ${currentWaveBomGuard};`);
    lines.push(`UPDATE projects SET project_kind = 'physical_design', updated_at = ${sqlString(now)} WHERE id = ${sqlString(row.id)} AND current_version_id = ${sqlString(row.current_version_id)} AND ${currentWaveBomGuard};`);
  }
  return `${lines.join('\n')}\n`;
}
