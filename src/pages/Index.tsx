  ...
  return <main>
    ...
    <div className="mx-auto max-w-[1400px] space-y-10 px-4 py-8">
      ...
      <section>
        ...
        {projects.isLoading ? <State>Loading projects…</State> : projects.error ? <State error>{projects.error.message}</State> : <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{projectRows.map((project) => <Link key={project.id} to={`/projects/${project.slug}`} className="surface-card surface-card-hover overflow-hidden"><div className="aspect-[16/7] bg-muted">{project.cover_image_url ? <img src={project.cover_image_url} alt="" className="h-full w-full object-cover" loading="lazy" /> : <div className="grid h-full place-items-center text-muted-foreground"><Package className="h-8 w-8 opacity-40" /></div>}</div><div className="p-3"><div className="flex items-start justify-between gap-3"><span className="font-semibold">{project.name}</span><span className={`pill ${project.license ? "pill-good" : "pill-yellow"}`}>{project.license ? project.license : "license unclear"}</span></div><p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{project.summary ?? "Source-linked robotics project. Structured summary enrichment is pending."}</p><div className="mt-3 grid grid-cols-3 gap-1 text-[10px] text-muted-foreground"><span>{project.rpps.bom?.length ?? 0} BOM lines</span><span>{project.githubStars?.toLocaleString() ?? "—"} stars</span><span>{project.publishability}</span></div></div></Link>)}</div>}
      </section>
