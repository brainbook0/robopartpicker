              ...
              <tbody>
                {offers.map((offer) => {
                  ...
                  return <tr key={offer.id}>
                    ...
                    <td>{offer.productUrl ? <a href={offer.productUrl} target="_blank" rel="noopener noreferrer" className="btn-ghost btn-sm inline-flex items-center gap-1"><ExternalLink className="h-3 w-3" /> Product page</a> : supplier?.website ? <a href={supplier.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[12px] text-muted-foreground hover:text-primary">{hostname(supplier.website)} <ExternalLink className="h-3 w-3" /></a> : <span className="text-muted-foreground">Unavailable</span>}</td>
                  </tr>;
                })}
