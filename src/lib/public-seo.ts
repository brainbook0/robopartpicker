...
export function canonicalBase(env: CanonicalEnv): string {
  ...
  const raw = configured || (env.CLOUDFLARE_ENV === "production" || env.NODE_ENV === "production" ? DEFAULT_PRODUCTION_BASE_URL : "http://localhost:5173");
  try {
...
export function replaceBase(content: string, base: string): string {
  return content.replaceAll(LEGACY_BASE_URL, base).replaceAll(DEFAULT_PRODUCTION_BASE_URL, base).replaceAll("http://localhost:5173", base);
}
