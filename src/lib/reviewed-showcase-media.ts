...
function originalGuard(project: PreparedReviewedShowcaseMedia): string {
  const { row } = project;
  return `p.id = ${sqlString(row.id)} AND p.slug = ${sqlString(row.slug)} AND p.current_version_id = ${sqlString(row.current_version_id)} AND p.project_kind = 'commercial_showcase' AND p.visibility = 'public' AND p.status = 'published' AND p.updated_at = ${sqlString(row.updated_at)} AND pv.id = p.current_version_id AND pv.project_id = p.id AND pv.rpps_json = ${sqlString(row.rpps_json)}`;
}
...
SELECT ${sqlString(project.evidenceId)}, 'docs', ${sqlString(definition.source_page_url)}, ${sqlString(`${definition.name} official product image`)}, ${sqlString(definition.source_publisher)}, ${sqlString(now)}, 0.99, ${sqlString(`sha256:${definition.sha256}`)}, ${sqlString(`Reviewed showcase cover from ${definition.source_publisher}; source image ${definition.final_source_image_url}`)}, ${sqlString(project.fileId)}, 0, ${sqlString(now)}
FROM projects p JOIN project_versions pv ON pv.id = p.current_version_id
...
WHERE id = ${sqlString(row.id)} AND slug = ${sqlString(row.slug)} AND current_version_id = ${sqlString(row.current_version_id)} AND project_kind = 'commercial_showcase' AND visibility = 'public' AND status = 'published' AND updated_at = ${sqlString(row.updated_at)}
...
export function buildReviewedShowcaseMediaRollbackSql(
  ...
  for (const project of [...projects].reverse()) {
    const { definition, row } = project;
    const currentGuard = `EXISTS (SELECT 1 FROM projects p JOIN project_versions pv ON pv.id = p.current_version_id WHERE p.id = ${sqlString(row.id)} AND p.slug = ${sqlString(row.slug)} AND p.current_version_id = ${sqlString(row.current_version_id)} AND p.project_kind = 'commercial_showcase' AND p.updated_at IN (${sqlString(row.updated_at)}, ${sqlString(now)}) AND pv.rpps_json IN (${sqlString(row.rpps_json)}, ${sqlString(project.nextRppsJson)}))`;
