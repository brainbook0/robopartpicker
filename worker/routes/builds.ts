...
const createSchema = z.object({ name: z.string().trim().min(2).max(120), description: z.string().trim().max(20_000).nullable().optional(), organizationId: z.string().uuid().nullable().optional(), sourceProjectId: z.string().uuid().nullable().optional(), visibility: z.enum(visibilities).default("private") }).strict();
const updateSchema = z.object({ version: z.number().int().positive(), name: z.string().trim().min(2).max(120).optional(), description: z.string().trim().max(20_000).nullable().optional(), organizationId: z.string().uuid().nullable().optional(), visibility: z.enum(visibilities).optional(), status: z.enum(buildStatuses).optional(), progressPercent: z.number().int().min(0).max(100).optional() }).strict();
