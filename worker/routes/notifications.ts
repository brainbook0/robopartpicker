...
const preferencesSchema = z.object({ notificationType: z.string().trim().min(1).max(100), inAppEnabled: z.boolean(), emailEnabled: z.boolean() }).strict();
...
notificationRoutes.get("/notifications/preferences", async (c) => { const rows = await c.env.DB.prepare(`SELECT notification_type AS notificationType, in_app_enabled AS inAppEnabled,
  email_enabled AS emailEnabled, updated_at AS updatedAt FROM notification_preferences WHERE user_id = ?1 ORDER BY notification_type`).bind(authenticatedUserId(c)).all(); return c.json({ items: rows.results }); });
notificationRoutes.put("/notifications/preferences", async (c) => { const userId = authenticatedUserId(c); const body = await parseJson(c, preferencesSchema); await c.env.DB.prepare(`INSERT INTO notification_preferences
  (user_id, notification_type, in_app_enabled, email_enabled, updated_at) VALUES (?1, ?2, ?3, ?4, ?5)
  ON CONFLICT(user_id, notification_type) DO UPDATE SET in_app_enabled = excluded.in_app_enabled, email_enabled = excluded.email_enabled, updated_at = excluded.updated_at`)
  .bind(userId, body.notificationType, body.inAppEnabled ? 1 : 0, body.emailEnabled ? 1 : 0, new Date().toISOString()).run(); return c.json({ updated: true }); });
