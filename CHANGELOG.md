...
- Removed the dead Discord community claim. `discord.gg/robopartpicker` returned Discord's "Unknown Invite" (code 10006) while three live surfaces advertised it: the site-wide footer, the homepage "Join the Discord to shape what the platform becomes" call to action, and the community page button. There is no Discord server. All three now point at the public repository's issue tracker, and `src/lib/site-config.ts` documents that a real invite belongs there once one exists. The footer's GitHub link also pointed at a 404ing organisation; it now points at the published repository.
