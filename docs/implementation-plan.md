...
- [x] Created and clean-applied twelve sequential D1 migrations: 128 tables including Better Auth/OAuth, all requested product domains, portable RPPS releases/collaboration, indexes, constraints, triggers, and FTS5 search.
...
- [~] Projects/RPPS/BOMs: the legacy flat RPPS record remains compatible, while RPPS 0.1 Draft adds vendor-neutral YAML manifest/lockfile schemas, legacy conversion, deterministic scorecards, anonymous validation/import analysis, bounded GitHub/RPPS/BOM/URDF/ZIP extraction, content-addressed immutable releases, exact-release build passports, evidence outcomes, structured proposals/review, and stable-ID semantic diffs. BOM versions/items/export/fork, organization scope, technical records, and R2 artifacts remain implemented. Pull-request synchronization and reusable assembly discovery remain subsequent slices.
...
- [x] The isolated live preview at <https://robopartpicker-preview.ludomi2502.workers.dev> uses dedicated Worker/D1/R2 resources. The catalog remains intentionally empty. Version 0.4.0, migrations 0011–0012, live OAuth/private MCP, SPA, empty-catalog, and deterministic importer smoke checks are applied and verified.
...
- [x] Record validated local states in annotated checkpoint tags through `v0.3.0-portable-rpps-foundation`; the 0.4.0 checkpoint is created after preview validation, while production deployment remains separate.
