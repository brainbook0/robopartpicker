...
- [~] External ingestion has a versioned JSON Schema/example, service credential, size/schema validation, partial success, idempotency, raw records, record-specific staging, exact matches, withdrawal, audit/errors, and admin review. Canonical create currently supports manufacturer, supplier, component, and evidence; the other accepted types remain staged for merge/review rather than direct creation.
