...
- [x] Created and clean-applied fourteen sequential D1 migrations covering Better Auth/OAuth, all requested product domains, portable RPPS releases/collaboration, cross-product AI/import/versioning/collaboration/analytics/operations records, indexes, constraints, triggers, FTS5 search, and narrative build descriptions.
...
- [~] Projects/RPPS/BOMs: the legacy flat RPPS record remains compatible, while RPPS 0.1 Draft adds vendor-neutral YAML manifest/lockfile schemas, legacy conversion, deterministic scorecards, anonymous validation/import analysis, bounded GitHub/RPPS/BOM/URDF/ZIP/mixed-file extraction, content-addressed immutable releases, prominent exact-release reproduction, aggregate reproduction outcomes, evidence levels, structured assembly/change proposals, stable-ID semantic diffs, lightweight URDF viewing, and dedicated parts/cost/time/software/assembly/integration/evidence detail routes. BOM versions/items/export/fork, organization scope, technical records, and R2 artifacts remain implemented. Pull-request synchronization and reusable assembly discovery remain subsequent slices.
...
- [~] Marketplace: persistent narrative-first drafts/editing/publishing, AI organization and advisory quality review, validated multi-image R2 media, seller/listing records, linked-build aggregate parts-cost versus asking-price context, saves, wanted types, inquiries/messages/offers/reports/moderation schema, detailed filters/sorts, build/component prefills, and honest no-payment claims are implemented. Transaction-provider, expiration automation, and full moderation UI are intentionally absent.
...
- [x] AI runs only server-side through a configurable provider/model registry and task router with budgets, timeouts, fallbacks, caching, prompt versions, usage/cost tracking, data-sensitivity policy, evaluation-gated activation, authorized evidence tools, contextual preview/apply/edit/reject/undo actions, and confirmation-gated high-impact proposals.
- [x] Stateless Streamable HTTP MCP exposes public read-only robotics search, component comparison, public project artifacts, and RPPS validation. OAuth-protected private MCP adds scoped project/release/build reads plus two-stage proposals, backed by PKCE, consent, JWT audience checks, and live-session revocation.
- [x] Persistent import jobs use Cloudflare Queues with retry/dead-letter handling. Worker-safe parsers handle bounded static formats; native CAD, archives, ROS tooling, and other untrusted processing route to the allowlisted Sandbox container without executing imported scripts or Xacro content.
...
- [x] Isolated Pixel 5 Playwright workflows validate mobile navigation, filter drawers, uploads/import progress, build checklists and Technical Records, part-out/listing creation, full-screen editor sizing/focus, messaging, improvement-record privacy, and administration denial against a fresh fourteen-migration D1 state.
...
- [x] The isolated live preview at <https://robopartpicker-preview.ludomi2502.workers.dev> uses dedicated Worker/D1/R2 resources. Version 0.5.0 (`156a4542-8bf8-4077-aa8a-322764e79cca`) and migration 0013 are deployed; the remote schema, health/API routes, public/private MCP boundaries, structured AI drafting/review, SPA deep links, and the single Berkeley Humanoid Lite import test record were checked. The catalog remains intentionally empty.
- [~] Repository-driven Playwright now verifies the primary mobile workflows and viewport/touch/focus acceptance checks on Pixel 5 emulation. Broader device coverage, deployed-environment browser checks, and a full accessibility audit remain release work.
...
- [x] Record validated local states in annotated checkpoint tags through `v0.5.0-reproduction-extraction-marketplace`; production deployment remains separate.
