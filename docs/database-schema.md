...
Files are named `0001_description.sql`, `0002_description.sql`, and so on. Applied migrations are immutable; corrections use new files. Schema migrations never include demo or production data. Clean migration, upgrade, and seed validation run in CI against Wrangler's local D1 state.
